  ...
  return <div>
    ...
    {order&&<>
      ...
      {plan&&<>
        ...
        {plan.shipping_summary?.complete&&!planLoading&&<ConfirmBoxUsage key={JSON.stringify([order?.name,packages])} packages={packages} reference={order?.name||orderNumber} custom={custom} packingResult={plan.loose_result} onToast={onToast}/>}
      </>}
    </>}
  </div>;
}
...
function PackageDatabaseView({ onToast, currentUser }) {
  ...
  function resetRecord(){setEdit(emptyRecord);setDimensionDraft(['','','']);}
  function changeUnit(unit){
    setDimensionDraft((edit.dimensions_in||['','','']).map(v=>dimensionForInput(v,unit)));
    setDimensionUnit(unit);
  }
  function changeDimension(index,value){
    setDimensionDraft(previous=>previous.map((v,i)=>i===index?value:v));
  ...
  async function del(){if(!edit.id)return;if(!window.confirm(`Delete package record ${edit.sku}${edit.part?` / ${edit.part}`:''}?`))return;try{await api.deleteShippingPackageRecord(edit.id);onToast?.('Package record deleted');resetRecord();await load();}catch(e){onToast?.(e.message,'error');}}
  return <div><div style={{display:'grid',gridTemplateColumns:'minmax(0,1.7fr) minmax(320px,.8fr)',gap:16,alignItems:'start'}}><div style={card}><div style={{padding:14,borderBottom:'1px solid var(--border)',display:'flex',gap:8}}><input value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')load();}} placeholder="Search SKU, product, or package part" style={inputStyle}/><button onClick={()=>load()} style={secondaryButton}>{loading?'Searching…':'Search'}</button></div><div style={{fontSize:11,color:'var(--text-light)',padding:'8px 12px'}}>{data?.count||0} matching records{data?.truncated?' · showing first 50 · search to narrow results':''}</div><div style={{overflow:'auto',maxHeight:650}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}><thead><tr style={{background:'#f8faf9'}}>{['SKU','Product / part','Dimensions','Status','Behavior'].map(h=><th key={h} style={th}>{h}</th>)}</tr></thead><tbody>{(data?.records||[]).map(r=><tr key={r.id} onClick={()=>choose(r)} style={{cursor:'pointer',background:edit.id===r.id?'#f0fbf4':'transparent'}}><td style={{...td,fontWeight:800}}>{r.sku}</td><td style={td}>{r.product_name}<div style={{fontSize:10,color:'var(--text-light)'}}>{r.part||'Primary package'}</div></td><td style={td}>{packageDimensionsText(r.dimensions_in,dimensionUnit)}<div style={{fontSize:10,color:'var(--text-light)'}}>{packageDimensionsText(r.dimensions_in,dimensionUnit==='cm'?'in':'cm')}</div></td><td style={td}><StatusBadge status={r.verification_status}/></td><td style={td}><BehaviorBadge behavior={r.shipping_behavior}/></td></tr>)}</tbody></table></div></div>
    <div style={{...card,padding:16,position:'sticky',top:12}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}><strong>{edit.id?'Edit package record':'Add package record'}</strong>{edit.id&&<button onClick={resetRecord} style={{...secondaryButton,padding:'5px 8px'}}>New</button>}</div><div style={{display:'grid',gap:9,marginTop:12}}><label style={{fontSize:11,fontWeight:800}}>SKU<input value={edit.sku||''} onChange={e=>field('sku',e.target.value)} style={{...inputStyle,marginTop:4}}/></label><label style={{fontSize:11,fontWeight:800}}>Product name<input value={edit.product_name||''} onChange={e=>field('product_name',e.target.value)} style={{...inputStyle,marginTop:4}}/></label><label style={{fontSize:11,fontWeight:800}}>Part<input value={edit.part||''} onChange={e=>field('part',e.target.value)} style={{...inputStyle,marginTop:4}}/></label><label style={{fontSize:11,fontWeight:800}}>Dimension units<select aria-label="Dimension units" value={dimensionUnit} onChange={e=>changeUnit(e.target.value)} style={{...inputStyle,marginTop:4}}><option value="cm">Centimetres (cm)</option><option value="in">Inches (in)</option></select></label><div style={{fontSize:11,color:'var(--text-light)'}}>Enter Shopify measurements directly in cm. Changing units converts the values automatically.</div><div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:6}}>{['L','W','H'].map((label,i)=><label key={label} style={{fontSize:11,fontWeight:800}}>{label} {dimensionUnit}<input type="number" min="0" step="any" value={dimensionDraft[i]} onChange={e=>changeDimension(i,e.target.value)} style={{...inputStyle,marginTop:4}}/></label>)}</div><div style={{fontSize:11,color:'var(--text-light)'}}>Equivalent: {packageDimensionsText(edit.dimensions_in,dimensionUnit==='cm'?'in':'cm')}</div><label style={{fontSize:11,fontWeight:800}}>Verification<select value={edit.verification_status||''} onChange={e=>field('verification_status',e.target.value)} style={{...inputStyle,marginTop:4}}><option>Verified — Warehouse</option><option>Confirmed — Vendor/Label</option><option>Shopify — Unverified</option></select></label><label style={{fontSize:11,fontWeight:800}}>Shipping behavior<select value={edit.shipping_behavior||'standard'} onChange={e=>field('shipping_behavior',e.target.value)} style={{...inputStyle,marginTop:4}}><option value="standard">Standard</option><option value="carrier_ready">Carrier-ready — ships in own package</option><option value="accessory_carrier">Accessory carrier — own package + may hold accessories</option><option value="must_ship_alone">Must ship alone</option></select></label><label style={{fontSize:11,fontWeight:800}}>Weight kg<input type="number" step="0.001" value={edit.weight_kg??''} onChange={e=>field('weight_kg',e.target.value)} style={{...inputStyle,marginTop:4}}/></label>{edit.shipping_behavior==='accessory_carrier'&&<label style={{fontSize:11,fontWeight:800}}>Carrier notes<textarea value={edit.carrier_notes||''} onChange={e=>field('carrier_notes',e.target.value)} rows="3" style={{...inputStyle,marginTop:4}}/></label>}<label style={{fontSize:11,fontWeight:800}}>Notes<textarea value={edit.notes||''} onChange={e=>field('notes',e.target.value)} rows="3" style={{...inputStyle,marginTop:4}}/></label><div style={{fontSize:10,color:'var(--text-light)'}}>Measured by: {edit.measured_by||currentUser||'Unknown'}</div><div style={{display:'flex',gap:8}}><button onClick={save} disabled={saving} style={{...primaryButton,flex:1}}>{saving?'Saving…':'Save Package'}</button>{edit.id&&<button onClick={del} style={{...secondaryButton,color:'#b91c1c'}}>Delete</button>}</div></div></div></div></div>;
}
